...
$.fn.newWindow = function(options){
	...
	// move($windowContainer, options.posx, $(window).scrollTop() + options.posy);
	...
	return this.each(function(index) {
		...
		$this.click(function(event){
			...
			if(options.ajaxURL != ""){ 
			     ...
			     if (true == ncw.REWRITE) {
			         var url = options.ajaxURL + '?_='+tsTimeStamp;
			     } else {
			         var url = options.ajaxURL + '&_='+tsTimeStamp;
			     }
			...
			move($window, options.posx, $(window).scrollTop() + options.posy);
			setFocus($window);
            $window.fadeIn();			
		});
	});
