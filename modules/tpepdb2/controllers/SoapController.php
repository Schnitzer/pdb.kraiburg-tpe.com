...
\t\t\tON brand.id = s.brand_id
        ...
        if (count($searchresult) > 0) {
            ...
            foreach ($searchresult as $result) {
                ...
                foreach ($arr_languages As $language) {
                    $arr_datashets[$language] = 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?sid=' . $sid . '&l=' . $language;
                }
    ...
    public function textSearchAction()
    {
        ...
        if (count($searchresult) > 0) {
            ...
            $arr_datashets = array();
            foreach ($arr_languages As $language) {
                $arr_datashets[$language] = 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?l=' . $language . '&sid=' . $result['s_id'];
            }
            foreach ($searchresult as $result) {
                $values['series'][] = array(
                    'id' => $result['s_id'],
        ...
        if ($just_count == 1) {
            ...
            if (count($searchresult) > 0 && count($searchresult) < $max_results) {
                ...
                foreach ($searchresult as $result) {
                    ...
                    $arr_datashets = array();
                    foreach ($arr_languages As $language) {
                        $arr_datashets[$language] = 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?l=' . $language . '&cid=' . $result['c_id'] . '&sessid=' . session_id();
                    }
                    // Compoundwerte
                    $values['compounds'][] = array(
                        ...
                        'safetydata_url' => 'http://drupaldev.kraiburg-tpe.com/tpepdb/safetydata?s=' . $result['safetydata'] . '&l=' . $lang_shortcut . '&t=' . $result['c_name'] . '&r=',
                        ...
                        'serie_datasheet_url' => 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?&sid=' . $result['s_id'] . '&l=' . $lang_shortcut,
                        'datasheet_url' => $arr_datashets,
                    ...
                    if ($s_id_tmp != $result['s_id']) {
                        ...
                        $arr_datashets = array();
                        foreach ($arr_languages As $language) {
                            $arr_datashets[$language] = 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?sid=' . $result['s_id'] . '&l=' . $language;
                        }
    ...
    protected function _getSeriesCompounds($sid, $language, $language_shortcut)
    {
        ...
        if (count($searchresult) > 0) {
            ...
            foreach ($searchresult as $result) {
                ...
                $arr_datashets = array();
                foreach ($arr_languages As $language) {
                    $arr_datashets[$language] = 'http://drupaldev.kraiburg-tpe.com/tpepdb/pdf?cid=' . $result['id'] . '&l=' . $language;
                }
                ...
                $values[] = array(
                    ...
                    'safetydata_url' => 'http://drupaldev.kraiburg-tpe.com/tpepdb/safetydata?s=' . $result['safetydata'] . '&l=' . $lang_shortcut . '&t=' . $result['c_name'] . '&r=1',
        ...
        if (count($arr_compound_document) > 0) {
            foreach ($arr_compound_document As $compounddocument) {
                ...
                if (true == strstr($obj_document->getName(), '.pdf')) {
                    $arr_return[] = array('url' => 'http://drupaldev.kraiburg-tpe.com/assets/tpepdb2/imported_documents/' . $obj_document->getName(), 'label' => $document_label);
                }
            }
        }
