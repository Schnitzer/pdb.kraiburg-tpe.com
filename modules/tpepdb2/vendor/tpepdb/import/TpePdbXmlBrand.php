...
class TpePdbXmlBrand extends TpePdbXmlItem {
    ...
    protected function _loadName ()
    {
        ...
        if (true === $this->is_portfolio) {
        	  //var_dump($this->_xml_element->{'BRAND'}[4]);
     ...
     *
     */
    protected function _loadZhName ()
    {
    	
    	//var_dump($this->_xml_element);
        if (true === $this->is_portfolio) {
        	  //var_dump($this->_xml_element->{'BRAND'}[0]);
            //$this->_name = trim((string) $this->_xml_element);
            $this->_name_zh = trim($this->_xml_element->{'BRAND'}[0]);
        } else {
            $this->_name_zh = trim((string) $this->_xml_element->DESCRIPTIONS_SHORT[0]->DESCRIPTION_SHORT[0]);
        }
    }
}
?>
