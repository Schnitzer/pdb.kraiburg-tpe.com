...
class TpePdbXmlImporter {
     ...
     * @static
     */
    public static $BRAND_XML_NODE_STRUCTURE = "CATALOG/NODES/NODE/ITEMS/ITEM/BRAND";
    ...
    public function __construct ()
    {
        if (true === isset($_GET['set_root_path']) && $_GET['set_root_path'] == 1) {
            self::$ROOT_PATH = '/usr/www/users/kraibn/';
        }
    ...
    public function import ()
    {
        ...
        $sth = $db->prepare("SELECT id, compound_id, internal_name FROM ncw_tpepdb2_processingnote");
        $sth->execute();
        $rows = $sth->fetchAll();
        foreach ($rows as $row) {
            self::$processing_notes[$row["compound_id"] . '_' . $row["internal_name"]] = $row["id"];
        }*/
