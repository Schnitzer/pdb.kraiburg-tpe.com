...
class TpePdbXmlItem {
                    ...
                    foreach ($attribute_values as $attribute_value) {
                        if (true === isset($attribute_value["lang"])
                            ...
                            foreach ($this->_languages as $language) {
                                ...
                                if (false !== strpos($attributes_xml[$i + 1]["internal_id"], "NORM")) {
                                    $norm = $attribute_name . "_" . str_replace(array("QPMK_", "DIN_"), "", (string) $attributes_xml[$i + 1]["internal_id"]);
                                    $this->addAttribute($norm, trim((string) $attributes_xml[$i + 1]->VALUE), "char", $attribute_lang, $attribute_num);
                                } else if (false !== strpos($attributes_xml[$i + 2]["internal_id"], "NORM")) {
                                    $norm = $attribute_name . "_" . str_replace(array("QPMK_", "DIN_"), "", (string) $attributes_xml[$i + 2]["internal_id"]);
                                    $this->addAttribute($norm, trim((string) $attributes_xml[$i + 2]->VALUE), "char", $attribute_lang, $attribute_num);
                                }
    ...
    protected function _importDocuments ()
    {
        ...
        $found_item_documents = $item_document_model->fetch(
            ...
            array(
                ...
                "conditions" => array(
                    $rel_item_name . "." . strtolower($this->_ncw_name) . "_id" => $this->model->getId(),
    ...
    {
        if (true === file_exists(TpePdbXmlImporter::$DOCUMENTS_SOURCE_DIR . DS . $document["name"])) {
            $src_dir = TpePdbXmlImporter::$DOCUMENTS_DEST_DIR . DS . $document["language"];
            if (false === is_dir($src_dir)) {
                mkdir($src_dir);
            }
            $document_path = TpePdbXmlImporter::$DOCUMENTS_SOURCE_DIR . DS . $document["name"];
            copy(
                $document_path,
                $src_dir . DS . $document["name"]
            );
