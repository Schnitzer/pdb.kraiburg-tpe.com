	...
	public static function addTTFfont($fontfile, $fonttype = '', $enc = '', $flags = 32, $outpath = '', $platid = 3, $encid = 1, $addcbbox = false, $link = false)
	{
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				// create CIDToGIDMap
				$cidtogidmap = str_pad('', 131072, "\0");  // (256 * 256 * 2) = 131072
	...
	public static function getUniord($uch)
	{
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if ($countbytes == 0) {  // get starting octect
				...
				} elseif (($char >> 0x5) == 0x6) {  // 2 bytes character (0x06 = 110 BIN)
					$bytes[] = ($char - 0xC0) << 0x6;
					++$countbytes;
					$numbytes = 2;
				} elseif (($char >> 0x4) == 0xE) {  // 3 bytes character (0x0E = 1110 BIN)
					$bytes[] = ($char - 0xE0) << 0xC;
					++$countbytes;
					$numbytes = 3;
				} elseif (($char >> 0x3) == 0x1E) {  // 4 bytes character (0x1E = 11110 BIN)
			...
			} elseif (($char >> 0x6) == 0x2) {  // bytes 2, 3 and 4 must start with 0x02 = 10 BIN
				$bytes[] = $char - 0x80;
				++$countbytes;
