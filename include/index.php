...
{
  $str = str_replace('http://drupaldev.kraiburg-tpe.com/pdb/en/products/product-database', 'http://kraiburg-tpe.com/special/curl', $str);

  $arr = explode('<!--TRENNER-->', $str);
  $arr1 = explode('<!--TRENNER-->', $arr[1]);

  $db_url = 'http://test.kraiburg-tpe.com/';
...
\t       <script src=\"" . $db_url . 'assets/tpepdb2/javascript/main.js?time=101609" type="text/javascript"></script>';
  ...
  $n = 'class="layout-content" style="background-image:url(https://2019.kraiburg-tpe.com/sites/default/files/assets/pattern/blue.png"';
...
$webseite_url = 'https://2019.kraiburg-tpe.com';
...
$code_pdb = getUrlContent('http://test.kraiburg-tpe.com' . $add_to_url);
$code_pdb = cleanPdb($code_pdb);
$code_pdb = str_replace('/sites/', $webseite_url . '/sites/', $code_pdb);
$code_pdb = str_replace('<script src="http://drupaldev.kraiburg-tpe.com/pdb/assets/wcms/js/plugins/lightbox/js/lightbox.min.js"></script>', '', $code_pdb);
