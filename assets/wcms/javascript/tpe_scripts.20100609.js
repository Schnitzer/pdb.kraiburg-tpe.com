...
$(document).ready(function () {
  ...
  $('#change_language li, #change_language_dropdown_icon').click(function () {
    $('#change_language_overlay').show();
    $('#change_language li').show();
    language_select_visible = true;
  });
  $('#change_language_overlay').click(function () {
  ...
  if ($.browser.mozilla) {
    $('#change_language_new li span').css('margin-top', '-6px');
  } else if ($.browser.webkit) {
    $('#change_language_new li span').css('margin-top', '0px');
 ...
 */
function search() {
  //var path = 'http://drupaldev.kraiburg-tpe.com';
  //var path = 'http://localhost/kraiburg-tpe';
  $.post(
    path + '/admin/wcms/website/search',
    $('#textsearchform').serialize(),
    function (data) {
      $('#ts_results').html('');
      $.each(data, function (site_id, attributes) {
        $('#ts_results').append('<div class="ts_box">' + '<h4><a href="' + attributes['url'] + '">' + attributes['name'] + '</a></h4>' + '<p>' + '<a href="' + attributes['url'] + '">' + attributes['content'] + '</a>' + '<br /><a href="' + attributes['url'] + '" class="url">' + attributes['url'] + '</a>' + '</p>' + '</div>');
      });
      if ($('#ts_results').html() == '') {
...
function search2015() {
  ...
  $.post(
    ...
    function (data) {
      ...
      $('#s_header').hide();
      $('#ts_form').hide();
      $('#s_section').css('height', '478px');
      $('#ts_results').css('height', '457px');
      $('#search_wrapper').show();
...
function region_search_country(region_id, label_country, label_forward, language) {
  ...
  str_button = '<div><input id="cf_btn_dbp" type="button" value="' + label_forward + '" class="tpe-button-small" tabindex="4" title="' + label_forward + "\" onclick=\"region_goto_site($('#cf_selectcountry_dbp').val(), $('#cf_selectcountry_dbp option:selected').attr('rel'));\" /></div>";
  //var path = 'http://drupaldev.kraiburg-tpe.com';
  //var path = 'http://localhost/kraiburg-tpe';
  $.getJSON(path + '/admin/tpe/region/ajaxCountries/' + region_id + '/' + language, function (data) {
    if (data.country) {
      str_options = '';
      $.each(data.country, function (i, item) {
        str_options += '<option value="' + item.url + '" rel="' + item.name + '" >' + item.name + '</option>';
      });
 ...
 * @param url
 */
function region_goto_site(url, regionsearch_countryname) {
  if (regionsearch_countryname.length > 1) {
    location.href = url + '?regionsearch_countryname=' + regionsearch_countryname;
  } else {
    location.href = url;
  }
}
