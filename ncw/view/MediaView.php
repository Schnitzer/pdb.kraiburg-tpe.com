 ...
 * @link      http://www.netzcraftwerk.com
 */
class Ncw_MediaView extends Ncw_View
{
     ...
     * @var array
     */
    public $mime_type = array('ai' => 'application/postscript', 'bcpio' => 'application/x-bcpio', 'bin' => 'application/octet-stream',
    ...
    {
        $name = $download = $extension = $id = $modified = $path = $size = $cache = $mime_type = null;
        ...
        if (true === is_dir($path)) {
            ...
            $path = ASSETS . DS . $this->module_name . DS . $path . DS . $id;
        }
        ...
        if (false === file_exists($path)) {
            ...
            if (DEBUG_MODE > 0) {
                throw new Ncw_Exception($message);
            } else {
                $header = new Ncw_Components_Header();
                $header = $header->object;
                $header->sendStatusCode(404);
                include_once $this->theme_path . DS . 'errors' . DS . 'error404.phtml';
            }
            $this->_stop();
        }
        ...
        if (true === isset($extension)
            && true === isset($this->mime_type[$extension])
            && connection_status() == 0
        ) {
            $chunkSize = 8192;
            $buffer = '';
            ...
            if ($download) {
                ...
                $this->_header(array(
                    ...
                    'Content-Type: ' . $this->mime_type[$extension],
