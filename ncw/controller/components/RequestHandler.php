...
class Ncw_Components_RequestHandler extends Ncw_Component
{
    ...
    public function startup(Ncw_Controller &$controller)
    {
        ...
        if (false === empty($this->ext) && $isRecognized) {
            $this->renderAs($controller, $this->ext);
        } elseif (true === $this->isAjax()) {
    ...
    public function renderAs(&$controller, $type)
    {
        ...
        if (Ncw_Configure::read('App.encoding') !== null) {
            $options = array('charset' => Ncw_Configure::read('App.encoding'));
        }

        if ($type == 'ajax') {
            $controller->layout = $this->ajax_layout;
    ...
    public function respondAs($type, $options = array())
    {
        ...
        if ($cType != null) {
            ...
            if (Ncw_Configure::read('debug_mode') < 2) {
                @header($header);
            }
